...
export const api = {
  ...
  games: {
    ...
    play: async (token: string, slug: string, betAmount: number, currency: 'GC' | 'SC' = 'GC') => {
      const response = await fetch(`${API_BASE_URL}/games/${slug}/play`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  // Wallet endpoints (True engine balances)
  wallet: {
