...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    // Generate verification token and a stable UUID for the True engine
    const verificationToken = crypto.randomBytes(32).toString('hex');
    const truePlayerId = crypto.randomUUID();
    ...
    // Provision a wallet in the True transaction engine.
    // Non-blocking: if True is down the user still registers and the wallet
    // can be created later (truePlayerId is already stored).
    trueWallet
      .createPlayer(truePlayerId, String(user._id), email, username)
      .catch((err) =>
        console.error('True wallet provisioning failed for', truePlayerId, ':', err?.message),
      );
