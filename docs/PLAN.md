...
| 6 | **Responsible gaming** | E G W | `[x]` **E**: migration `000011_responsible_gaming`, engine guards self-exclusion / cool-off / loss limit / deposit limit inside the wallet-locked transaction for `/spin`, `/bet`, `/win`, `/rollback`, `/store/purchase`; signed `POST /player/limits` (set/lift), `/player/limits/query`, and the pre-charge `/player/limits/check-purchase` (True#21). Migration `000012_rg_counters_reset` fixed the loss limit to count SC play only, not GC. | Session limits, reality-check reminders. Gateway: store settings, call `/player/limits/check-purchase` before charging a card, refund if `/store/purchase`'s backstop ever refuses. Web: `/account/limits` page. |
