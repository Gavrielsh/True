...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (!token) return;
      try {
        const [txData, walletData] = await Promise.allSettled([
          api.transactions.getAll(token),
          api.wallet.getBalances(token),
        ]);
        if (txData.status === 'fulfilled') setTransactions(txData.value);
        if (walletData.status === 'fulfilled') setWalletBalances(walletData.value?.balances ?? null);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 py-12 px-4">
      <div className="container mx-auto max-w-7xl">
        ...
        {/* True Wallet Breakdown */}
        {walletBalances && (
          <div className="mb-6 bg-gray-800/50 backdrop-blur-lg border border-purple-500/20 rounded-xl p-6">
            <h2 className="text-lg font-bold text-white mb-4">Transaction Engine Wallet</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                <div className="text-green-300 text-sm font-medium mb-1">🟡 Gold Coins (GC)</div>
                <div className="text-white text-2xl font-bold">{parseFloat(walletBalances.gc).toFixed(2)}</div>
                <div className="text-gray-400 text-xs mt-1">Real money play balance</div>
              </div>
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                <div className="text-blue-300 text-sm font-medium mb-1">🔵 SC Unplayed</div>
                <div className="text-white text-2xl font-bold">{parseFloat(walletBalances.sc_unplayed).toFixed(2)}</div>
                <div className="text-gray-400 text-xs mt-1">Promo coins (must be wagered)</div>
              </div>
              <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                <div className="text-purple-300 text-sm font-medium mb-1">🟣 SC Redeemable</div>
                <div className="text-white text-2xl font-bold">{parseFloat(walletBalances.sc_redeemable).toFixed(2)}</div>
                <div className="text-gray-400 text-xs mt-1">Won coins (redeemable)</div>
              </div>
            </div>
          </div>
        )}
